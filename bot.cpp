...
int main() {
    ...
    std::pair <int,int>card_value = {};
    std::pair <int,int>dealer_card_value = {};
    int card = 0;
    ...
    bot.on_ready([&](const dpp::ready_t & event) {
        if (dpp::run_once<struct register_bot_commands>()) {
            ...
            greeting.add_option(//an option is a parameter
            ...
            //this part is not done yet, feel free to rewrite the this part
    ...
    bot.on_slashcommand([&bot, &card_value, &gameStarted, &dealer_card_value, &card, &guess_ans](const dpp::slashcommand_t & event) {
        ...
        else if (event.command.get_command_name() == "hit") {
            dpp::message m;
            int get_card = rand() % 13 + 1;
            int card_sum = 0;
    ...
    else if (event.command.get_command_name() == "stand") {
        dpp::message m;
    ...
    else if (com == "greeting") {
            
            std::string username = std::get<std::string>(event.get_parameter("username"));
            event.reply("Hello, " + username);
        }
        else if (com == "add") {
            //int is a datatype in cpp, co_integer is a type for a parameter in dpp.
            //I end up using string, int causes "bad variant access"
